...
trait JSSemantics extends sc.imp.machine.SemanticsBase {
  this: Terms with States with Envs with Stores with Continuations =>
  ...
  def getPropertyAddress(loc: HeapLoc, i: Value, σ: Store) = {
    σ.get(loc) match {
