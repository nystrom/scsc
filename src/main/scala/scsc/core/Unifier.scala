...
trait Unifier[Exp] extends Subst[Exp] {
  ...
  /*
    def unify(t1: Cont, t2: Cont): Option[Subst] = {
      val s = currSubst
      mguk(applySubst(s)(t1), applySubst(s)(t2)) match {
        case Some(s) =>
          currSubst = s @@ currSubst
          Some(currSubst)
        case None =>
    ...
    def mgu(e1: Exp, e2: Exp): Option[Subst] = {
      ...
      if (es1.length == es2.length) {
        (es1 zip es2).foldLeft (Some(emptySubst): Option[Subst]) {
          case (None, _) => None
          case (Some(s), (name(x1), name(x2))) if x1 == x2 => Some(s)
          case (Some(s), (name(x1), name(x2))) if x1 < x2 => Some(s @@ singletonSubst(x1, name(x2)))
          case (Some(s), (name(x1), name(x2))) if x1 > x2 => Some(s @@ singletonSubst(x2, name(x1)))
          case (Some(s), (name(x1), e2: Exp)) => Some(s @@ singletonSubst(x1, e2))
          case (Some(s), (e1, e2)) if e1 == e2 => Some(s)
